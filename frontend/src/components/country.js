...
function createPolygon(polygonCoords, radius = 1) {
  const vertices = [];
  const holes = [];
  ...
  polygonCoords.forEach((ring, index) => {
    ...
    ring.forEach(([lon, lat]) => {
      const vertex = convertCoordsTo3D(lat, lon, radius);
...
function Country() {
  const mesh = createPolygon(southAfricanGeoJSON.geometry.coordinates);
